...
class E160_robot:
    ...
    def __init__(self, environment, address, robot_id):
        ...
        self.file_name = 'Log/Bot9' + str(self.robot_id) + '_' + datetime.datetime.now().replace(microsecond=0).strftime('%y-%m-%d %H.%M.%S') + '.csv'
        self.make_headers()
    ...
    def point_tracker_control(self):
        ...
        if not self.point_tracked:
            ...
            control = self.velocity_to_wheel_control(desiredVelocityL, desiredVelocityR)
            ...
            self.data = [self.state_des.x, self.state_est.x, self.state_des.y, self.state_est.y, self.state_des.theta, self.state_est.theta, pho, alpha, beta, self.Kpho, self.Kalpha, self.Kbeta, desiredV, desiredW]
    ...
    def velocity_to_wheel_control(self, lv, rv):
        deltaEML = self.encoder_measurements[1] - self.last_encoder_measurements[1]
        if deltaEML == 0:
        ...
        deltaEMR = self.encoder_measurements[0] - self.last_encoder_measurements[0]
        if deltaEMR == 0:
    ...
    def make_headers(self):
        ...
        f.write(','.join(['time','state_des.x', 'state_est.x', 'state_des.y', 'state_est.y', 'state_des.theta', 'state_est.theta', 'pho', 'alpha', 'beta','Kpho', 'Kalpha', 'Kbeta', 'desiredV', 'desiredW']) + '\n')
        f.close()
    ...
    def update_odometry(self, encoder_measurements):
        ...
        wheelDistanceL = float(deltaEML) / float(self.encoder_resolution) * float(self.wheel_circ)
        wheelDistanceR = float(deltaEMR) / float(self.encoder_resolution) * float(self.wheel_circ)
    ...
    def update_state(self, state, delta_s, delta_theta):
        ...
        x = self.state_est.x + delta_s * math.cos(self.state_est.theta + delta_theta/2)
        y = self.state_est.y + delta_s * math.sin(self.state_est.theta + delta_theta/2)
